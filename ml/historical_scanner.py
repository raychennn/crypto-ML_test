...
def _scan_symbol_worker(args: tuple) -> List[dict]:
    ...
    try:
        ...
        for start_idx in range(0, n - ref_len - max_forward, stride):
...
class HistoricalScanner:
    ...
    def scan_reference(
        ...
        # Load reference data
        ref_df = self.data_manager.read_symbol(
            ref.symbol, ref.timeframe,
            start_ts=ref.start_ts, end_ts=ref.end_ts
        )
        if ref_df.empty or len(ref_df) < 10:
            logger.warning(f"Reference {ref.id}: insufficient data ({len(ref_df)} bars)")
            return pd.DataFrame()
        ...
        # Prepare reference z-normalized close
        ref_close = ref_df["close"].values
        ...
        for sym, sym_matches in matches_by_symbol.items():
            ...
            # Pre-compute SMAs for the entire symbol once
            sym_with_sma = sym_df.copy()
            for period in self.config.sma_periods:
                sym_with_sma[f"SMA_{period}"] = (
                    sym_with_sma["close"].rolling(period).mean()
                )
