...
class ChartGenerator:
    ...
    def generate_alert_chart(
        ...
        Args:
            window_df: 匹配區間的 OHLCV DataFrame
            symbol: e.g. "SOLUSDT"
            timeframe: e.g. "4h"
    ...
    def generate_reference_chart(
        ...
        Args:
            window_df: The reference pattern OHLCV DataFrame
            symbol: e.g. "AVAXUSDT"
            timeframe: e.g. "4h"
        ...
        if "datetime" in df.columns:
            dt_start = pd.Timestamp(df["datetime"].iloc[0]).strftime("%Y-%m-%d %H:%M")
            dt_end = pd.Timestamp(df["datetime"].iloc[-1]).strftime("%Y-%m-%d %H:%M")
            date_text = f"{dt_start}  →  {dt_end}  |  {len(df)} bars"
    ...
    def generate_reference_sma_chart(
        ...
        for period in self.config.sma_periods:
            col = f"SMA_{period}"
            sma = df[col] if col in df.columns else df["close"].rolling(window=period, min_periods=1).mean()
            color = COLOR_SMA.get(period, "#ffffff")
            ax.plot(dates, sma.values, color=color, linewidth=2.0,
        ...
        if len(df) >= 2:
            dt_start = pd.Timestamp(df["datetime"].iloc[0]).strftime("%Y-%m-%d %H:%M")
            dt_end = pd.Timestamp(df["datetime"].iloc[-1]).strftime("%Y-%m-%d %H:%M")
            ax.set_title(f"{dt_start}  →  {dt_end}  |  {len(df)} bars",
    ...
    def _plot_sma_lines(self, ax, df: pd.DataFrame):
        ...
        for period in self.config.sma_periods:
            ...
            color = COLOR_SMA.get(period, "#ffffff")
            ax.plot(dates, sma.values, color=color, linewidth=1.2,
