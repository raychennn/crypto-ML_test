    ...
    def scan_timeframe(
        ...
        for ref in tf_refs:
            ...
            if ref_df.empty or len(ref_df) < 10:
                logger.warning(
                    f"Reference {ref.id}: insufficient data ({len(ref_df)} bars)"
                )
                continue

            ref_prepared = self.normalizer.prepare_for_dtw(ref_df)
            ref_prepared["timeframe"] = timeframe

            logger.info(
                f"Scanning ref={ref.id} ({len(ref_df)} bars) "
            ...
            for match in matches:
                ml_probs = self.predictor.predict(
                    match, match.window_data, btc_df
                )
                alert = AlertResult(
                    symbol=match.symbol,
                    timeframe=timeframe,
                ...
                if r.symbol in candidates:
                    ...
                    if cand_df is not None:
                        r.window_data = cand_df.iloc[
                            r.match_start_idx:r.match_end_idx
                        ].copy()
                matches.append(r)
